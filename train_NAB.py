...
import copy 
import json
from dataset_origin import LoadDataset_NAB as LoadDataset_NAB_origin
from dataset_origin import FeatDataLayer as FeatDataLayer_origin
from dataset import FeatDataLayer, LoadDataset_NAB, FeatDataLayer_add_FG
...
parser.add_argument('--exp_no',  type=str, default="")
parser.add_argument('--bird_lambda',  type=float, default=0.6)
parser.add_argument('--family_lambda',  type=float, default=0.1)
parser.add_argument('--genus_lambda',  type=float, default=0.3)
...
opt.BIRD_CENT_LAMBDA = opt.bird_lambda
opt.FAMILY_CENT_LAMBDA = opt.family_lambda
opt.GENUS_CENT_LAMBDA = opt.genus_lambda
...
def train():
    ...
    dataset_origin = LoadDataset_NAB_origin(opt)
    print("load dataset")
    dataset = LoadDataset_NAB(opt)
    param.X_dim = dataset.feature_dim

    data_layer_origin = FeatDataLayer_origin(dataset_origin.labels_train, dataset_origin.pfc_feat_data_train, opt)
    data_layer = FeatDataLayer_add_FG(dataset.labels_train, dataset.pfc_feat_data_train, opt,
                                      ...
                                      dataset.labels_origin_train, dataset="NAB")
    result = Result()
    result_gzsl = Result()
    ...
    opt.log_dir = log_dir
    opt.auc_plot_dir = out_subdir + '/best_auc_plot{:s}_{:s}'.format(opt.exp_no, exp_info)

    opt.auc_solid_plot_dir = out_subdir + '/solid_auc_plot{:s}_{:s}'.format(opt.exp_no, exp_info)

    opt.history_D_loss_dir = out_subdir + '/D_loss_plot{:s}_{:s}'.format(opt.exp_no, exp_info)
    opt.history_G_loss_dir = out_subdir + '/G_loss_plot{:s}_{:s}'.format(opt.exp_no, exp_info)
    ...
    for it in range(start_step, 10000+1):
        ...
        for _ in range(5):
            ...
            text_feat = np.array([dataset_origin.train_text_feature[i, :] for i in labels])
        ...
        for _ in range(1):
            ...
            text_feat = blobs['text_feat']
            text_feat = Variable(torch.from_numpy(text_feat.astype('float32'))).cuda()

            X = Variable(torch.from_numpy(feat_data)).cuda()
            y_true = Variable(torch.from_numpy(labels.astype('int'))).cuda()
            y_origin_true = Variable(torch.from_numpy(origin_labels.astype('int'))).cuda()
            y_true = y_true.to(device=device, dtype=torch.long)
            z = Variable(torch.randn(opt.batchsize, param.z_dim)).cuda()
            ...
            cur_G_loss += -G_loss.item()+F.cross_entropy(C_fake, y_true).item()
            ...
            if opt.CENT_LAMBDA != 0 and opt.BIRD_CENT_LAMBDA != 0:
                for i in range(dataset.train_cls_num):
                    ...
                    if sample_idx.numel() == 0:
                        ...
                        center = Variable(torch.from_numpy(dataset.tr_cls_centroid[i].astype('float32'))).cuda()
            ...
            if opt.CENT_LAMBDA != 0 and opt.FAMILY_CENT_LAMBDA!= 0:
                for i in range(dataset.familyLabelStart, dataset.familyLabelEnd):
                    ...
                    if sample_idx.numel() == 0:
                        ...
                        G_sample_cls = G_sample[sample_idx, :]
                        center = Variable(torch.from_numpy(dataset.tr_cls_centroid[i].astype('float32'))).cuda()
                        Family_Euclidean_loss += (G_sample_cls.mean(dim=0) - center).pow(2).sum().sqrt()
                Family_Euclidean_loss *= 1.0 / (dataset.familyLabelEnd - dataset.familyLabelStart)
            ...
            if opt.CENT_LAMBDA != 0 and opt.GENUS_CENT_LAMBDA != 0:
                for i in range(dataset.genusLabelStart, dataset.genusLabelEnd):
                    ...
                    if sample_idx.numel() == 0:
                        ...
                        G_sample_cls = G_sample[sample_idx, :]
                        center = Variable(torch.from_numpy(dataset.tr_cls_centroid[i].astype('float32'))).cuda()
                        Genus_Euclidean_loss += (G_sample_cls.mean(dim=0) - center).pow(2).sum().sqrt()
                Genus_Euclidean_loss *= 1.0 / (dataset.genusLabelEnd - dataset.genusLabelStart)
        ...
        print("Iter-" + str(it + 1) + "; G-loss: " + str(cur_G_loss) + "; D-loss: " + str(cur_D_loss))
        ...
        if it % opt.evl_interval == 0 and it >= 100:
            ...
            if result.save_model:
                files2remove = glob.glob(out_subdir + '/Best_model{}_Acc*'.format(opt.exp_no))
                ...
                torch.save({
                    ...
                    'log': log_text,
                }, out_subdir + '/Best_model{}_Acc_{:.2f}.tar'.format(opt.exp_no, result.acc_list[-1]))

            if result_gzsl.save_model:
                files2remove = glob.glob(out_subdir + '/Best_model{}_Auc*'.format(opt.exp_no))
                for _i in files2remove:
                    os.remove(_i)
                torch.save({
                    'it': it + 1,
                    ...
                    'random_seed': opt.manualSeed,
                    'log': log_text,
                }, out_subdir + '/Best_model{}_Auc_{:.2f}.tar'.format(opt.exp_no, result_gzsl.best_auc * 100))
    ...
    print("Reproduce NAB {}".format(opt.splitmode))
    print("Accuracy is {:.4}%, and Generalized AUC is {:.4}% \n".format(result.best_acc, result_gzsl.best_auc * 100))
...
def eval_fakefeat_test(it, netG, dataset, param, result):
    ...
    try:
        ...
        # cosince predict K-nearest Neighbor
        sim = cosine_similarity(dataset.pfc_feat_data_train, gen_feat)
        idx_mat = np.argsort(-1 * sim, axis=1)
        label_mat = (idx_mat[:, 0:opt.Knn] / opt.nSample).astype(int)
        preds = np.zeros(label_mat.shape[0])
        for i in range(label_mat.shape[0]):
    ...
    if train_acc >= result.train_best_acc:
        ...
        result.test_best_acc = acc
        opt.flag = True

    print("{}nn Classifier: ".format(opt.Knn))
    print("test: Accuracy is {:.4}%; Train Accuracy is {:.4}%".format(acc, train_acc))
...
def eval_fakefeat_GZSL(it, netG, dataset, param, result):
    ...
    if opt.flag == True:
        result.test_best_auc = auc_score
        opt.flag = False

        plt.plot(acc_S_T_list, acc_U_T_list)
        plt.title("CUB-{:s}: {:.4}%".format(opt.splitmode, auc_score * 100))
        plt.savefig(opt.auc_solid_plot_dir + '.png')
        plt.clf()
        plt.close()
        np.savetxt(opt.auc_solid_plot_dir + '.txt', np.vstack([acc_S_T_list, acc_U_T_list]))
    ...
    if auc_score > result.best_auc:
        ...
        plt.plot(acc_S_T_list, acc_U_T_list)
        plt.title("NAB-{:s}: {:.4}%".format(opt.splitmode, auc_score * 100))
        plt.savefig(opt.auc_plot_dir + '.png')
        plt.clf()
        plt.close()
        np.savetxt(opt.auc_plot_dir + '.txt', np.vstack([acc_S_T_list, acc_U_T_list]))
...
class Result(object):
    def __init__(self):
        ...
        self.train_best_auc = 0.0
        self.test_best_acc = 0.0
        self.test_best_auc = 0.0
